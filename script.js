// ============================================
// NOBUQR.SPACE - SCRIPT v2.0 (ПОЛНЫЙ)
// ============================================
...
async function submitChirp() {
    ...
    if (selectedMediaFile) {
        ...
        var { error: uploadError } = await supabase.storage.from(bucket).upload(fileName, selectedMediaFile);
        if (uploadError) { showToast('Ошибка загрузки файла', 'error'); btn.disabled = false; btn.textContent = 'Опубликовать'; return; }
    ...
    var { data: chirp, error } = await supabase.from('chirps').insert({
        user_id: currentUser.id, content: filterContent(content), media_url: mediaUrl,
        media_type: mediaType, hashtags: hashtags
    }).select('*, users:user_id (username, handle, avatar_url, is_verified)').single();
    if (error) { showToast('Ошибка при публикации', 'error'); btn.disabled = false; btn.textContent = 'Опубликовать'; return; }
    await supabase.from('users').update({ chirps_count: (currentUser.chirps_count || 0) + 1 }).eq('id', currentUser.id);
    currentUser.chirps_count += 1;
    input.value = ''; removeMedia(); updateCharCount();
    btn.disabled = false; btn.textContent = 'Опубликовать';
    prependChirp(chirp);
    addGems(1, 'earn', 'За пост');
...
function buildChirpHTML(c) {
    ...
    return '<div class="chirp-card-header"><div class="chirp-avatar" onclick="event.stopPropagation(); window.openProfile(\'' + u.id + '\')">' +
        ...
        '<button class="chirp-action-btn" onclick="event.stopPropagation(); window.openChirp(\'' + c.id + '\')">💬 <span class="chirp-action-count">' + (c.comments_count || 0) + '</span></button>' +
        '<button class="chirp-action-btn">🔄 <span class="chirp-action-count">0</span></button>' +
        '<button class="chirp-action-btn" onclick="event.stopPropagation(); window.reportChirp(\'' + c.id + '\')">🚩</button></div>';
}
...
function renderChirps(chirps, container) {
    ...
    var html = '';
    for (var i = 0; i < chirps.length; i++) html += buildChirpHTML(chirps[i]);
...
// ========================
// КОММЕНТАРИИ, ПРОФИЛИ, УВЕДОМЛЕНИЯ
// ========================
async function openChirp(chirpId) {
    ...
    if (!chirp) return;
    var content = '<div class="chirp-card">' + buildChirpHTML(chirp).replace(/onclick="event.stopPropagation(); window.openChirp/g, 'onclick="') + '</div><div class="comments-section"><div class="create-post-container"><div class="create-post-card"><div class="create-post-avatar"><img src="' + (currentUser.avatar_url || '') + '"></div><div class="create-post-input-area"><textarea id="commentInput" class="create-post-input" placeholder="Комментарий..."></textarea><button class="create-post-submit" onclick="window.submitComment(\'' + chirpId + '\')">Ответить</button></div></div></div><div id="commentsList">Загрузка...</div></div>';
...
async function loadProfileData(userId) {
    ...
    if (!isOwn && currentUser) {
        ...
        followBtn = '<button class="btn btn-primary btn-small" onclick="window.toggleFollow(\'' + userId + '\')">' + (f && f.length > 0 ? 'Отписаться' : 'Подписаться') + '</button>';
    }
    var html = '<div class="profile-header"><div class="profile-banner">' + (user.banner_url ? '<img src="' + user.banner_url + '">' : '') + '</div><div class="profile-avatar-section"><div class="profile-avatar-large"><img src="' + (user.avatar_url || '') + '"></div>' + followBtn + '</div><div class="profile-info"><div class="profile-display-name">' + user.username + (user.is_verified ? ' <span class="chirp-verified">✓</span>' : '') + '</div><div class="profile-handle">@' + user.handle + '</div>' + (user.bio ? '<div class="profile-bio">' + user.bio + '</div>' : '') + '<div class="profile-meta"><span>📅 ' + new Date(user.created_at).toLocaleDateString() + '</span></div><div class="profile-stats"><div class="profile-stat"><strong>' + (user.followers_count || 0) + '</strong> <span>подписчиков</span></div><div class="profile-stat"><strong>' + (user.following_count || 0) + '</strong> <span>подписок</span></div><div class="profile-stat"><strong>' + (user.chirps_count || 0) + '</strong> <span>постов</span></div></div></div></div><div id="profileChirps" class="chirps-list">Загрузка...</div>';
...
function reportChirp(chirpId) {
    openModal('Жалоба', '<p>Причина:</p><select id="reportReason" class="form-input"><option value="spam">Спам</option><option value="harassment">Оскорбление</option><option value="inappropriate">Неприемлемый контент</option></select><textarea id="reportDetails" class="form-input mt-2" placeholder="Подробности"></textarea>', true, function() {
        var reason = document.getElementById('reportReason').value;
        var details = document.getElementById('reportDetails').value;
        submitReport(chirpId, reason, details);
    });
}
async function submitReport(chirpId, reason, details) {
    ...
    await supabase.from('reports').insert({ reporter_id: currentUser.id, reported_user_id: chirp ? chirp.user_id : null, chirp_id: chirpId, reason: details || reason, report_type: reason });
...
async function openNotificationsModal() {
    ...
    if (!notifications || notifications.length === 0) html = '<div class="empty-state"><div class="empty-state-icon">🔔</div><div class="empty-state-title">Нет уведомлений</div></div>';
        ...
        for (var i = 0; i < notifications.length; i++) {
            ...
            var icon = n.type === 'like' ? '❤️' : (n.type === 'comment' ? '💬' : (n.type === 'follow' ? '👤' : '⚠️'));
            html += '<div class="notification-item ' + (n.is_read ? '' : 'unread') + '" onclick="window.markNotifRead(\'' + n.id + '\', \'' + (n.chirp_id || '') + '\')"><div class="notification-icon">' + icon + '</div><div class="notification-content"><strong>' + n.from_user.username + '</strong> ' + (n.type === 'like' ? 'понравился ваш пост' : (n.type === 'comment' ? 'прокомментировал ваш пост' : (n.type === 'follow' ? 'подписался на вас' : 'предупреждение'))) + '<div class="notification-time">' + getTimeAgo(new Date(n.created_at)) + '</div></div></div>';
        }
    }
...
function renderAdminScreen() {
    if (!isAdmin) { var p = prompt('Пароль админа:'); if (p !== 'N0buSp@ce2024') { showToast('Неверно', 'error'); renderScreen('main'); return; } isAdmin = true; }
...
async function loadAdminStats() {
    var { count: u } = await supabase.from('users').select('*', { count: 'exact', head: true });
    var { count: c } = await supabase.from('chirps').select('*', { count: 'exact', head: true });
    var { count: r } = await supabase.from('reports').select('*', { count: 'exact', head: true }).eq('status', 'pending');
    document.getElementById('adminStats').innerHTML = '<div class="admin-stat-card"><div class="admin-stat-value">' + (u||0) + '</div><div class="admin-stat-label">Пользователей</div></div><div class="admin-stat-card"><div class="admin-stat-value">' + (c||0) + '</div><div class="admin-stat-label">Постов</div></div><div class="admin-stat-card"><div class="admin-stat-value">' + (r||0) + '</div><div class="admin-stat-label">Жалоб</div></div>';
}
async function loadAdminUsers() {
    var { data: users } = await supabase.from('users').select('*').order('created_at', { ascending: false }).limit(50);
    var html = '<table class="admin-table"><tr><th>Пользователь</th><th>Статус</th><th>Пред.</th><th>Действия</th></tr>';
...
async function loadAdminReports() {
    var { data: reports } = await supabase.from('reports').select('*, reporter:reporter_id (username), reported:reported_user_id (username)').order('created_at', { ascending: false }).limit(20);
...
async function warnUser(userId) {
    ...
    if (newWarnings >= user.max_warnings) {
        ...
        updateData.ban_reason = 'Лимит предупреждений';
...
async function resolveReport(reportId, action) {
    if (action === 'warn') {
        var { data: r } = await supabase.from('reports').select('reported_user_id').eq('id', reportId).single();
        if (r && r.reported_user_id) await warnUser(r.reported_user_id);
    }
...
async function addGems(amount, type, reason, relatedUserId) {
    if (!currentUser) return;
    var newBalance = Math.max((currentUser.gems || 0) + amount, 0);
    await supabase.from('gems').update({ balance: newBalance, total_earned: (currentUser.gems || 0) + (amount > 0 ? amount : 0), total_spent: (currentUser.gems || 0) + (amount < 0 ? -amount : 0) }).eq('user_id', currentUser.id);
...
async function buyItem(itemId) {
    ...
    if (!item) return;
    if (currentUser.gems < item.price) { showToast('Недостаточно 💎', 'error'); return; }
    if (item.item_type !== 'lootbox' && item.item_type !== 'boost') {
        ...
        if (existing) { showToast('Уже есть этот предмет', 'warning'); return; }
    }
    ...
    if (item.item_type === 'lootbox') {
        ...
        var postsHtml = '<p>Выберите пост для продвижения:</p><div id="promoPostsList">Загрузка...</div>';
        openModal('Выбор поста', postsHtml, true, function() {
            var selected = document.querySelector('input[name="promoPost"]:checked');
            if (selected) activatePromotion(selected.value, hours, superPromo);
...
function renderInventoryScreen() {
    var html = '<div class="screen-main"><header class="topbar"><div class="topbar-content"><button class="btn-icon" onclick="window.renderScreen(\'main\')">← Назад</button><h1 class="topbar-title">Инвентарь</h1></div></header><div class="inventory-container" id="inventoryItems"></div></div>';
    appContainer.innerHTML = html;
    loadInventory();
}
async function loadInventory() {
    ...
    for (var i = 0; i < inv.length; i++) {
        var entry = inv[i], item = entry.item;
        html += '<div class="market-item"><div class="market-item-icon">' + getItemIcon(item.item_type) + '</div><div class="market-item-info"><div class="market-item-name">' + item.name + '</div></div><div class="market-item-actions">' + (entry.is_equipped ? '<button class="btn-unequip" onclick="window.unequipItem(\'' + entry.id + '\')">Снять</button>' : '<button class="btn-equip" onclick="window.equipItem(\'' + entry.id + '\')">Надеть</button>') + '</div></div>';
    }
    container.innerHTML = html;
}
...
// ========================
// ПРАВИЛА, КОНФИДЕНЦИАЛЬНОСТЬ
// ========================
function renderRulesScreen() {
    appContainer.innerHTML = '<div class="legal-page"><button class="legal-back-btn" onclick="window.goBack()">← Назад</button><h1>Правила использования</h1><p>Запрещены оскорбления, спам, материалы 18+, нарушение авторских прав. Администрация может блокировать аккаунты без объяснения причин.</p></div>';
}
function renderPrivacyScreen() {
    appContainer.innerHTML = '<div class="legal-page"><button class="legal-back-btn" onclick="window.goBack()">← Назад</button><h1>Политика конфиденциальности</h1><p>Мы храним email, имя, хешированные пароли и контент. Данные не передаются третьим лицам. Вы можете запросить удаление аккаунта.</p></div>';
}
