...
var developmentActive = false;

// Пароль: nobu2025
var ADMIN_HASH = sha512_256('nobu2025');
...
function hideAll() {
    ['feed-container','profile-container','settings-container'].forEach(function(id) {
        document.getElementById(id).style.display = 'none';
    });
}

// Сессия
function saveSession(user) { currentUser = user; localStorage.setItem('nobuqr_session', JSON.stringify({user:user, expires_at:Date.now()+86400000})); }
function loadSession() {
    ...
    localStorage.removeItem('nobuqr_session'); return false;
}
    ...
    if (currentFeed === 'subscriptions') {
        supabase.from('follows').select('followee_id').eq('follower_id', currentUser.id).then(function(fr) {
            if (fr.data && fr.data.length) {
                ...
                supabase.from('chirps').select('*, users!inner(id, username, handle, avatar_url)').in('user_id', ids).order('created_at', false).range(feedPage*10, feedPage*10+9).then(handleFeed);
            } else {
        ...
        return;
    } else q = supabase.from('chirps').select('*, users!inner(id, username, handle, avatar_url)').order('created_at', false).range(feedPage*10, feedPage*10+9).then(handleFeed);
}
...
function renderPosts(posts) {
    ...
    posts.forEach(function(p) {
        ...
        card.innerHTML = '<div class="post-header" onclick="viewProfile(\''+u.id+'\')" style="cursor:pointer;">' +
            '<div class="post-avatar" style="background-image:url('+(u.avatar_url||'')+')"></div>' +
            '<div><span class="post-author">'+escapeHtml(u.username)+'</span> <span class="post-handle">'+escapeHtml(u.handle)+'</span></div>' +
            '</div>' +
...
function renderProfileData(u) {
    ...
    supabase.from('chirps').select('*,users(*)').eq('user_id',u.id).order('created_at',false).then(function(p) {
        ...
        if (p.data) p.data.forEach(function(post) {
            var card = document.createElement('div'); card.className = 'post-card';
            card.innerHTML = '<div class="post-header" onclick="viewProfile(\''+post.users.id+'\')" style="cursor:pointer;"><div class="post-avatar" style="background-image:url('+(post.users.avatar_url||'')+')"></div><div><span class="post-author">'+escapeHtml(post.users.username)+'</span></div></div><div class="post-content">'+escapeHtml(post.content)+'</div>'+(post.media_url?'<img src="'+post.media_url+'" class="post-media">':'');
            c.appendChild(card);
        });
    });
}
...
function showAdminPanel() {
    if (localStorage.getItem('admin_session') === 'true') { openModal('admin-modal'); switchAdminTab('stats'); }
    else openModal('admin-login-modal');
}
function loginAdmin() {
    var pwd = document.getElementById('admin-password').value;
    var hash = sha512_256(pwd);
    if (hash === ADMIN_HASH) {
...
function switchAdminTab(tab) {
    ...
    tabs.forEach(function(t,i) { t.classList.toggle('active', i===['stats','users','gems','posts','verifications','maintenance','development'].indexOf(tab)); });
    ...
    else if (tab==='development') loadDevelopmentTab();
}
function loadAdminStats() { Promise.all([ supabase.from('users').select('*',{count:'exact',head:true}), supabase.from('chirps').select('*',{count:'exact',head:true}) ]).then(function(r) { document.getElementById('admin-content').innerHTML = 'Пользователей: '+r[0].count+' | Чирпов: '+r[1].count; }); }
function loadAdminUsers() { supabase.from('users').select('*').then(function(r) { var h = ''; r.data.forEach(function(u) { h += '<div style="padding:8px;border-bottom:1px solid var(--border)">'+u.handle+' <button onclick="banUser(\''+u.id+'\')" class="btn btn-sm">Бан</button> <button onclick="requestVerification(\''+u.id+'\')" class="btn btn-sm">Проверка</button></div>'; }); document.getElementById('admin-content').innerHTML = h; }); }
...
function loadVerifications() { supabase.from('age_verifications').select('*,users(handle)').order('created_at',false).then(function(r) { var h = '<h3>Проверки возраста</h3>'; if (r.data && r.data.length) r.data.forEach(function(v) { h += '<div>'+v.users.handle+' - '+v.status+' <button onclick="approveVerification(\''+v.id+'\')">Одобрить</button> <button onclick="rejectVerification(\''+v.id+'\',\''+v.user_id+'\')">Отклонить</button></div>'; }); else h += '<p>Нет запросов</p>'; document.getElementById('admin-content').innerHTML = h; }); }
...
// Тех. перерыв и разработка
function loadMaintenanceTab() {
    var h = '<h3>Технический перерыв</h3>';
    h += '<p>Сейчас: ' + (maintenanceActive ? 'ВКЛЮЧЕН' : 'ВЫКЛЮЧЕН') + '</p>';
    h += '<button onclick="toggleMaintenance()" class="btn">' + (maintenanceActive ? 'Выключить' : 'Включить') + '</button>';
    document.getElementById('admin-content').innerHTML = h;
}
function loadDevelopmentTab() {
    var h = '<h3>Разработка NobuChirp</h3>';
    h += '<p>Сейчас: ' + (developmentActive ? 'ВКЛЮЧЕН' : 'ВЫКЛЮЧЕН') + '</p>';
    h += '<button onclick="toggleDevelopment()" class="btn">' + (developmentActive ? 'Выключить' : 'Включить') + '</button>';
    document.getElementById('admin-content').innerHTML = h;
}
function toggleMaintenance() {
    maintenanceActive = !maintenanceActive;
    if (maintenanceActive) developmentActive = false;
    supabase.channel('system').send({ type: 'broadcast', event: 'maintenance', payload: { active: maintenanceActive } });
    loadMaintenanceTab();
}
function toggleDevelopment() {
    developmentActive = !developmentActive;
    if (developmentActive) maintenanceActive = false;
    supabase.channel('system').send({ type: 'broadcast', event: 'development', payload: { active: developmentActive } });
    loadDevelopmentTab();
}

// Проверка возраста
function startAgeVerification() { var s = document.getElementById('verification-status'); s.textContent = 'Проверка браузера...'; setTimeout(function() { s.textContent = 'Сканирование лица...'; setTimeout(function() { supabase.from('age_verifications').update({status:'awaiting_admin'}).eq('user_id',currentUser.id).eq('status','pending').then(function() { s.textContent = 'Данные отправлены.'; setTimeout(closeModal, 2000); }); }, 3000); }, 2000); }
...
function setupRealtime() {
    ...
    realtimeChannels = [
        ...
        supabase.channel('system').on('broadcast', { event: 'maintenance' }, function(payload) {
            if (payload.active) openModal('maintenance-modal');
            else closeModal();
        }).on('broadcast', { event: 'development' }, function(payload) {
            if (payload.active) openModal('development-modal');
            else closeModal();
        }).subscribe()
    ];
}
...
function showPolicy(type) {
    var titles = { terms:'Правила NobuSocial', privacy:'Конфиденциальность', cookies:'Cookie' };
    var content = type === 'terms' ? '<p>Запрещены спам, оскорбления, контент 18+. Администрация может блокировать аккаунты без объяснения причин.</p>' :
                  type === 'privacy' ? '<p>Мы собираем email, имя, handle. Для пользователей до 13 лет требуется дополнительная проверка возраста. Данные не передаются третьим лицам. Вы можете удалить аккаунт в настройках.</p>' :
                  '<p>Cookie используются для сессии. Продолжая использовать NobuSocial, вы соглашаетесь с этим.</p>';
...
// Мобильная навигация
function mobileNav(s) { hideAll(); if (s==='feed') showFeed('latest'); else if (s==='profile') viewProfile(currentUser.id); }
