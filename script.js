...
// Проверка статуса системы (тех. перерыв / разработка)
function checkSystemStatus() {
    var mode = localStorage.getItem('system_mode');
    if (mode === 'maintenance') {
        openModal('maintenance-modal');
    } else if (mode === 'development') {
        openModal('development-modal');
    }
    // Проверяем каждые 5 секунд
    setInterval(function() {
        var currentMode = localStorage.getItem('system_mode');
        if (currentMode === 'maintenance' && document.getElementById('maintenance-modal').style.display !== 'block') {
            openModal('maintenance-modal');
        } else if (currentMode === 'development' && document.getElementById('development-modal').style.display !== 'block') {
            openModal('development-modal');
        } else if (!currentMode) {
            closeModal();
        }
...
// Тех. перерыв и разработка (через localStorage)
function loadMaintenanceTab() {
    var isOn = localStorage.getItem('system_mode') === 'maintenance';
    var h = '<h3>Технический перерыв</h3>';
    h += '<p>Сейчас: ' + (isOn ? 'ВКЛЮЧЕН' : 'ВЫКЛЮЧЕН') + '</p>';
    h += '<button onclick="toggleMaintenance()" class="btn">' + (isOn ? 'Выключить' : 'Включить') + '</button>';
    document.getElementById('admin-content').innerHTML = h;
}
function loadDevelopmentTab() {
    var isOn = localStorage.getItem('system_mode') === 'development';
    var h = '<h3>Разработка NobuChirp</h3>';
    h += '<p>Сейчас: ' + (isOn ? 'ВКЛЮЧЕН' : 'ВЫКЛЮЧЕН') + '</p>';
    h += '<button onclick="toggleDevelopment()" class="btn">' + (isOn ? 'Выключить' : 'Включить') + '</button>';
    document.getElementById('admin-content').innerHTML = h;
}
function toggleMaintenance() {
    ...
    if (isOn) {
        ...
        localStorage.setItem('system_mode', 'maintenance');
    }
    loadMaintenanceTab();
    // Применяем сразу
    if (!isOn) {
...
function toggleDevelopment() {
    ...
    if (isOn) {
        ...
        localStorage.setItem('system_mode', 'development');
    }
    loadDevelopmentTab();
    if (!isOn) {
        openModal('development-modal');
