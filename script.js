...
// Хэширование пароля (SHA-512/256)
...
function loadFeed() {
    ...
    if (currentFeed === 'popular') {
        query = supabase.from('chirps').select('*, users!inner(id, handle, username, avatar_url, is_premium)').order('created_at', { ascending: false }).range(feedPage * 10, feedPage * 10 + 9);
        ...
        supabase.from('follows').select('followee_id').eq('follower_id', currentUser.id).then(function(followRes) {
            if (followRes.data && followRes.data.length > 0) {
                ...
                supabase.from('chirps').select('*, users!inner(id, handle, username, avatar_url, is_premium)').in('user_id', ids).order('created_at', { ascending: false }).range(feedPage * 10, feedPage * 10 + 9).then(function(res) {
    ...
    if (query) {
        query.then(function(res) {
            ...
            showToast('Ошибка загрузки ленты: ' + err.message, 'error');
            document.getElementById('feed-loader').style.display = 'none';
            feedLoading = false;
        });
    }
...
function performSearch() {
    ...
    if (query.startsWith('@')) {
        supabase.from('users').select('*').ilike('handle', '%' + query.substring(1) + '%').then(function(res) {
            var html = '';
...
function sendMessage() {
    ...
    if (currentChatId) {
        ...
        supabase.from('group_messages').insert({ group_id: currentGroupId, sender_id: currentUser.id, content: text }).then(function() {
            input.value = '';
...
function mobileNav(section) {
    hideAllContainers();
    if (section === 'feed') showFeed(currentFeed);
...
function showPolicy(type) {
    ...
    if (type === 'terms') {
        title = 'Правила использования NobuSocial';
        content = '<h3>1. Общие положения</h3><p>Регистрируясь, вы соглашаетесь соблюдать законы РФ и не нарушать права других пользователей.</p>' +
            '<h3>2. Запрещённый контент</h3><p>Запрещены: спам, порнография, угрозы, оскорбления, разжигание ненависти.</p>' +
            '<h3>3. Возрастные ограничения</h3><p>Сервис предназначен для лиц старше 13 лет. Мы соблюдаем COPPA.</p>' +
            '<h3>4. Ответственность</h3><p>Администрация не несёт ответственности за пользовательский контент, но оставляет за собой право удалять его и блокировать аккаунты.</p>' +
            '<p><strong>Нарушение правил ведёт к бану без предупреждения.</strong></p>';
    } else if (type === 'privacy') {
        title = 'Политика конфиденциальности';
        content = '<h3>1. Сбор данных</h3><p>Мы собираем: email, имя пользователя, @handle, публикуемый контент, IP-адрес, файлы cookie.</p>' +
            '<h3>2. Использование данных</h3><p>Данные используются для работы сервиса, персонализации, безопасности и улучшения платформы.</p>' +
            '<h3>3. Хранение и защита</h3><p>Данные хранятся в зашифрованном виде на серверах Supabase. Мы принимаем все меры для их защиты.</p>' +
            '<h3>4. Передача третьим лицам</h3><p>Мы не продаём данные. Раскрываем только по требованию закона.</p>' +
            '<h3>5. Удаление аккаунта</h3><p>Вы можете удалить аккаунт в настройках, после чего ваши личные данные будут удалены.</p>' +
            '<p><strong>Мы соблюдаем GDPR и COPPA.</strong></p>';
    } else if (type === 'cookies') {
        title = 'Политика Cookie';
        content = '<h3>Что такое cookie?</h3><p>Небольшие файлы для сохранения настроек и сессии.</p>' +
            '<h3>Какие мы используем</h3><p>Сессионные cookie для входа, функциональные для ежедневного бонуса.</p>' +
            '<p><strong>Продолжая использовать сайт, вы соглашаетесь на использование cookie.</strong></p>';
    }
