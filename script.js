...
function renderPosts(posts) {
    ...
    posts.forEach(function(p) {
        ...
        card.innerHTML = '<div class="post-header"><div class="post-avatar" style="background-image:url(' + (u.avatar_url||'') + ')"></div><div><span class="post-author">' + escapeHtml(u.username) + '</span> <span class="post-handle">' + escapeHtml(u.handle) + '</span></div></div>' +
            ...
            '<div class="post-actions"><button class="action-btn" onclick="likeChirp(\'' + p.id + '\',this)">Нравится 0</button><button class="action-btn" onclick="rechirp()">Поделиться</button></div>';
        c.appendChild(card);
    });
}
...
function viewProfile(uid) {
    ...
    var now = Date.now();
    if (profileCache[uid] && (now - profileCache[uid].timestamp) < 30000) {
...
function checkDailyBonus() {
    ...
    if (last !== today) {
        ...
        giveGems(amount, 'Ежедневный бонус (день ' + count + ')');
...
function openShop() { supabase.from('shop_items').select('*').then(function(r) { var c = document.getElementById('shop-items'); c.innerHTML = ''; document.getElementById('shop-gems-count').textContent = currentUser.gems||0; if (r.data) r.data.forEach(function(i) { c.innerHTML += '<div class="shop-item"><span>'+i.name+'</span><span>'+i.price+' Gems</span><button class="btn btn-sm" onclick="buyItem(\''+i.id+'\','+i.price+')">Купить</button></div>'; }); }); openModal('shop-modal'); }
...
function loadAdminUsers() { supabase.from('users').select('*').then(function(r) { var h = ''; r.data.forEach(function(u) { h += '<div style="display:flex;justify-content:space-between;padding:8px;border-bottom:1px solid #ddd">'+u.handle+' <button onclick="banUser(\''+u.id+'\')" class="btn btn-admin btn-sm">Бан</button> <button onclick="requestVerification(\''+u.id+'\')" class="btn btn-sm">Запросить проверку</button></div>'; }); document.getElementById('admin-content').innerHTML = h; }); }
function requestVerification(uid) {
    supabase.from('age_verifications').insert({ user_id: uid, status: 'pending' }).then(function() { showToast('Запрос отправлен', 'success'); });
}
function loadVerifications() {
    supabase.from('age_verifications').select('*, users(handle)').order('created_at', false).then(function(r) {
        var h = '<h3>Запросы проверки возраста</h3>';
        if (r.data && r.data.length) {
            r.data.forEach(function(v) {
                h += '<div style="margin:8px 0;padding:8px;border:1px solid #ddd">' +
                    '<strong>' + v.users.handle + '</strong> – ' + v.status +
                    ' <button onclick="approveVerification(\'' + v.id + '\')" class="btn btn-sm">Одобрить</button>' +
                    ' <button onclick="rejectVerification(\'' + v.id + '\', \'' + v.user_id + '\')" class="btn btn-sm btn-admin">Отклонить</button>' +
                    '</div>';
            });
...
function approveVerification(verId) {
    supabase.from('age_verifications').update({ status: 'approved', updated_at: new Date() }).eq('id', verId).then(function() { showToast('Проверка одобрена', 'success'); loadVerifications(); });
}
function rejectVerification(verId, userId) {
    supabase.from('age_verifications').update({ status: 'rejected', updated_at: new Date() }).eq('id', verId).then(function() {
        supabase.from('bans').insert({ user_id: userId, reason: 'Отклонена проверка возраста (нет 13 лет)', banned_until: '2099-01-01T00:00:00Z' }).then(function() {
            showToast('Отклонено, аккаунт забанен', 'success');
            loadVerifications();
        });
    });
}
...
function loadAdminPosts() { supabase.from('chirps').select('*, users(handle)').order('created_at',false).limit(50).then(function(r) { var h = ''; r.data.forEach(function(p) { h += '<div style="display:flex;justify-content:space-between;padding:8px;border-bottom:1px solid #ddd">'+p.content.substring(0,30)+'... <button onclick="deletePost(\''+p.id+'\')" class="btn btn-admin btn-sm">X</button></div>'; }); document.getElementById('admin-content').innerHTML = h; }); }
...
function setupRealtime() {
    ...
    realtimeChannels = [
        ...
        supabase.channel('age_verif').on('postgres_changes', {event:'INSERT', schema:'public', table:'age_verifications'}, function(p) {
            if (p.new.user_id === currentUser.id && p.new.status === 'pending') {
...
function startAgeVerification() {
    ...
    setTimeout(function() {
        ...
        setTimeout(function() {
            supabase.from('age_verifications').update({ status: 'awaiting_admin', updated_at: new Date() }).eq('user_id', currentUser.id).eq('status', 'pending').then(function() {
                statusEl.textContent = 'Данные отправлены администратору. Ожидайте решения.';
                document.getElementById('verification-steps').innerHTML = '<p>Спасибо! Ожидайте решения администратора.</p>';
...
function showPolicy(type) {
    ...
    var cont = type === 'terms' ? '<p>Правила использования...</p>' : type === 'privacy' ? '<p>Политика конфиденциальности (включая дополнительную проверку возраста)</p>' : '<p>Cookie...</p>';
    document.getElementById('policy-content').innerHTML = cont;
