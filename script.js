(function() {
    ...
    const DOM = {
        ...
        modalNickname: $('modalNickname'), modalBio: $('modalBio'),
    ...
    async function loadProfile(userId = null) {
        ...
        const loaded = data || { nickname: (currentUser?.email?.split('@')[0]) || 'Гость', bio: '', role: 'user', created_at: new Date().toISOString() };
    ...
    function updateAllUI() {
        ...
        DOM.userInitial.textContent = nick.charAt(0).toUpperCase();
        DOM.composerInitial.textContent = nick.charAt(0).toUpperCase();
        // Аватарки убраны – показываем инициалы
        DOM.userAvatar.style.backgroundImage = '';
        DOM.userAvatar.classList.remove('has-image');
    ...
    async function showList(title, queryField, queryValue) {
        ...
        if (profiles) {
            profiles.forEach(p => {
                ...
                div.innerHTML = `<div class="modal-list-avatar">${(p.nickname?.charAt(0)||'?').toUpperCase()}</div><span class="modal-list-name">${esc(p.nickname||'Без ника')}</span>`;
    ...
    async function openProfile(userId) {
        ...
        DOM.profileContainer.innerHTML = `
            <div class="profile-header-card" data-user-id="${userId}">
                <div class="profile-avatar-large">${(prof.nickname || '?').charAt(0).toUpperCase()}</div>
